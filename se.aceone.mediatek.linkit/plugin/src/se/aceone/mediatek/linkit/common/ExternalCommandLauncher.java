...
public class ExternalCommandLauncher {
	...
	private class LogStreamRunner implements Runnable {
		...
		public void run() {
			try {
				...
				IStatus status = new Status(IStatus.ERROR, LinkItConst.CORE_PLUGIN_ID, "I/O Error reading output", e);
				Common.log(status);
			} finally {
				try {
					fReader.close();
	 ...
	 *            all arguments
	 */
	public ExternalCommandLauncher(String command, IProject project) {
	...
	public ExternalCommandLauncher(List<String> command, IProject project) {
		this.project = project;
		Assert.isNotNull(command);
		fRunLock = this;
	...
	public int launch(IProgressMonitor monitor) throws IOException {
		...
		try {
			...
			fProcessBuilder.directory(new File(project.getLocation().toOSString()));
